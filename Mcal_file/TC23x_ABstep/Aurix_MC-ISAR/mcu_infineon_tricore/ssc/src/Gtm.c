...
void Gtm_IsrTomModule(uint8 ModuleNo, uint8 ChannelNumber)
{
  ...
  #if (GTM_MCUSAFETY_ENABLE == STD_ON)
  boolean IsTomChSpuriousInterrupt = TRUE;
  #endif
  ...
  TomModuleUsage = (uint32)Gtm_kConfigPtr->GtmModuleConfigPtr->                \
    ...
    do
    {
      ...
      if(Gtm_lGetIntSource(IrqStatus, ChanIndex))
      {
        ...
        #if (GTM_MCUSAFETY_ENABLE == STD_ON)
        /* current Interrupt is not a spurious interrupt*/
        IsTomChSpuriousInterrupt = FALSE;
        #endif
    ...
    #if (GTM_MCUSAFETY_ENABLE == STD_ON)
    /* if current interrupt is a spurious interrupt then report safety error */
    if(IsTomChSpuriousInterrupt == TRUE)
    {
       SafeMcal_ReportError ((uint16)GTM_MCU_MODULE_ID,GTM_MCU_MODULE_INSTANCE,
                     GTM_SID_TOM_ISR,GTM_E_SPURIOUS_INTRPT);
    }
    else
    {
      /* Do Nothing */
