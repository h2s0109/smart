...
/*  TRACEABILITY : [cover parentID=SAS_AS_CRC022,SAS_AS_CRC024,AUTOSAR:12911,
                    ...
                    SAS_NAS_ALL_PR1652,DS_AS_CRC005,DS_AS_CRC013,
                    DS_AS_CRC036,DS_AS_CRC037,DS_AS_CRC039,DS_AS_CRC023,
                    DS_NAS_CRC_PR228,DS_AS_CRC023,DS_AS_CRC009,
                    DS_NAS_CRC_PR912,DS_NAS_CRC_PR123,
                    SAS_NAS_ALL_PR630_PR631,DS_NAS_CRC_PR730,
                    DS_AS403_CRC041,
                    SAS_NAS_ALL_PR128,SAS_MCAL_CRC_0446,
                    SAS_MCAL_CRC_0448]
                    [/cover]                                                 */
/*******************************************************************************
**                      Includes                                              **
*******************************************************************************/
  ...
  /*Inclusion of Mcal Compiler header file*/
...
/* Memory Map of the CRC Code */
 ...
 is allowed only for MemMap.h*/
  ...
  #ifdef CRC_32_MODE
    #if  CRC_32_MODE == CRC_INSTRUCTION_MODE 
       static uint32 Crc_lCalculateCRC32Instruction( const uint8* Crc_DataPtr,
...
/* Memory Map of the CRC Code */
  #define CRC_START_SEC_CODE
  /*IFX_MISRA_RULE_19_01_STATUS=IFile inclusion after pre-processor directives
   is allowed only for MemMap.h*/
  #include "MemMap.h"
...
** Traceability   : [cover parentID=DS_AS_CRC002,DS_AS_CRC015,DS_AS403_CRC019,
    DS_AS_CRC002,DS_AS_CRC003,DS_AS_CRC015,DS_MCAL_CRC_0447_3]
[/cover]
  ...
  )
  {
    ...
    #endif/* #if (CRC_SAFETY_ENABLE == STD_ON) */
    {
      ...
      Crc32Result = Crc_lCalculateCRC32Table( Crc_DataPtr, Crc_Length,
                    Crc_StartValue32, Crc_IsFirstCall);
      #elif CRC_32_MODE == CRC_INSTRUCTION_MODE
      Crc32Result = Crc_lCalculateCRC32Instruction( Crc_DataPtr, Crc_Length,
                    Crc_StartValue32, Crc_IsFirstCall);
      #endif
   }   
   return Crc32Result;
...
*******************************************************************************/
  ...
  /* code for crc 8 Table based calculaion for 0x1D polynomials*/
   ...
   #endif 
  /* #endif for (CRC_8_MODE == CRC_TABLE_MODE)*/
  #endif
...
*******************************************************************************/
  ...
  #endif
  /* #endif for (CRC_16_MODE == CRC_TABLE_MODE)*/
  #endif
...
**                                                                           **
**                                                                           **
** Syntax     : static uint32 Crc_lCalculateCRC32Instruction                 **
**      (                                                                    **
**        const uint8* Crc_DataPtr,                                          **
**        uint32 Crc_Length,                                                 **
**        uint32 Crc_StartValue32,                                           **
...
** Description    :                                                          **
** -This function takes the value of input data pointer, start value and     **
** polynomial, calculates the crc using the Instruction Based and returns the**
**   CRC reversed and xored with the XOR_VALUE                               **
** - This function is local to the library module and is not for the user of **
**   CRC Library                                                             **
******************************************************************************/
  static uint32 Crc_lCalculateCRC32Instruction( const uint8* Crc_DataPtr, 
...
/* if it is first call assign initial value to the start value */
/*CRC Intrinsic Function will by default take Initial Value as
  0xFFFFFFFF.Hence we initialize Crc_StartValue32 to 0x00
  Crc Intrinsic function will do a xor with 0xffffffff
*/
    if (Crc_IsFirstCall == (boolean)TRUE)
    {
      Crc_StartValue32 = CRC_INIT_VAL;
    }
    for (LoopCounter = CRC_LOOPCNT_MSGBYTE_INIT_VAL; LoopCounter < Crc_Length;
    ...
    element of data*/
      ...
      Crc_StartValue32 = (uint32)CRC32((uint32)Crc_StartValue32, 
                     (uint32)tempVal); 
}
       
    return Crc_StartValue32;
  }
   #endif /*CRC_INSTRUCTION_MODE*/
