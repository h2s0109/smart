    ...
    def clockxl_load(self, xlpath):
        ...
        for j in range(1,ActivateSheet.max_column):
            for i in range(1,ActivateSheet.max_row):
                if re.match('Mcu*', str(ActivateSheet.cell(row=i, column=j).value)) is not None:
                    if isinstance(ActivateSheet.cell(row=i, column=j+1).value, int):
                            #change from normal to scientific
                            if re.search('Frequency', ActivateSheet.cell(row=i, column=j).value) is not None:
                                temp_float = float(ActivateSheet.cell(row=i, column=j+1).value)
                                temp_float = re.sub('\+0','',str("{:.1E}".format(temp_float)))
                                ConfigItem_dict[ActivateSheet.cell(row=i, column=j).value].update({'value':temp_float})
                            else:              
                                ConfigItem_dict[ActivateSheet.cell(row=i, column=j).value].update({'value':str(ActivateSheet.cell(row=i, column=j+1).value)})
                            ConfigItem_dict[ActivateSheet.cell(row=i, column=j).value].update({'row':i})
                            ConfigItem_dict[ActivateSheet.cell(row=i, column=j).value].update({'column':j})
                            ConfigItem_list.append(ActivateSheet.cell(row=i, column=j).value)
                    else:
                        #Special case
                        if re.fullmatch(ActivateSheet.cell(row=i, column=j+1).value ,'NOMAL') is True:
                            ConfigItem_dict[ActivateSheet.cell(row=i, column=j).value].update({'value':'NORMAL_MODE'})
                            ConfigItem_dict[ActivateSheet.cell(row=i, column=j).value].update({'row':i})
                            ConfigItem_dict[ActivateSheet.cell(row=i, column=j).value].update({'column':j})
                            ConfigItem_list.append(ActivateSheet.cell(row=i, column=j).value)
                        else:
