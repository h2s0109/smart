...
from PyQt5.QtGui import QStandardItem, QStandardItemModel, QPainter, QFont
from PyQt5.QtWidgets import QApplication, QDialog, QStyledItemDelegate, QStyleOptionViewItem, QFileDialog
...
class ImageDialog(QDialog, Ui_Dialog):
    ...
    def __init__(self):
        ...
        self.DATABASE_PATH  = os.path.realpath("{0}/Database/sort_key.json".format(THIS_DIR))
        self.TEMP_DATABASE_PATH  = os.path.realpath("{0}/Database/data_key.json".format(THIS_DIR))
        ...
        self.McalComboBox.currentIndexChanged.connect(lambda: self.ComboxBoxClick(self.McalComboBox))
        self.McalDirButton.clicked.connect(lambda: self.setExistingDirectory(self.McalComboBox, 'Mcal'))
        self.ModuleDirButton.clicked.connect(lambda: self.setExistingDirectory(self.ModuleComboBox, 'Module'))
        self.SmoduleDirButton.clicked.connect(lambda: self.setExistingDirectory(self.SmoduleComboBox, 'Smodule'))
        self.SetModuleButton.clicked.connect(lambda: self.SetModuleTreeViewUpdate(self.RelieveModuleModel,self.SetModuleModel))
        self.RelieveModuleButton.clicked.connect(lambda: self.SetModuleTreeViewUpdate_Relieve(self.SetModuleModel, self.RelieveModuleModel))
    ...
    def setExistingDirectory(self, comboboxname, targetcombox):
        ...
        tempfolder = self.settings.value(targetcombox+'/RECENTFOLDERLIST', [])
        ...
        if self.CopyProcessValid is True:
            ...
            for k_num in range(0, SetModuleParent.rowCount()):
                ...
                for j_num in range(0,rowCount_child):
                    index_basemodule_child = SetModuleParent.index(j_num, 0, index_basemodule)
                    item = SetModuleParent.itemFromIndex(index_basemodule_child)
                    ...
                    if item_child.checkState() == Qt.Checked:
                        ...
                        data_child = SetModuleParent.data(index_basemodule_child)
            ...
            for k_num in range(0, parentCount):
                ...
                if not SetModuleParent.hasChildren(index_basemodule):
                    ...
                    if tempCount is parentCount:
                        self.CopyProcessValid_NoSelect = True
                        self.plainTextEdit_Install.appendPlainText("All modules are unselected")
        else:
        ...
        if self.CopyProcessValid is True:
            ...
            for k_num in range(0, Relieve_parent.rowCount()):
                ...
                for j_num in range(0,Relieve_parent.rowCount(index_basemodule)):
                    index_basemodule2 = Relieve_parent.index(j_num, 0, index_basemodule)
    ...
    def CopyProcess(self):
        ...
        if self.CopyProcessValid is True and self.CopyProcessValid_NoSelect is False:
            ...
            if Temp_Module and Temp_Smodule is not "":
                ...
                includelist = self.includepath(Temp_Module,Temp_Smodule, include)
                self.plainTextEdit.clear()
                for k_list in includelist:
                    self.plainTextEdit.appendPlainText(k_list)                    
            else:
                self.plainTextEdit.setPlainText("Warning: There are no destination")
        else:
            self.plainTextEdit.setPlainText("Warning: Modules are not configured")
        return
            ...
            if len(tmp_file):
                for filelist in tmp_file:
                    if match('^.*\.h',filelist):
                        ...
                        print('{}{}{}'.format('${workspace_loc:/${ProjName}/{TEMPLETE}/',sub(delete_path,'',tmp_dir),'}'))
                        includelist.append('"${workspace_loc:/${ProjName}/{TEMPLETE}/'+ sub(delete_path,'',tmp_dir) +'}"')
    ...
    def McalFolderHandlingProcedure(self, directory):
        ...
        displasydata = dict()
        displasydata.update({"MODULEDATA":{"BASICMODULE":bmodule_data, "SERVICEMODULE":smodule_data, "MODULELIST":[]}})
        ...
        if QFile.exists(self.TEMP_DATABASE_PATH):
            ...
            RelieveModuleTreeInform = self.RelieveModuleBuildtree(self.RelieveModuleModel, modulelist, basemodule)
            ...
            SetModuleTreeInform = self.SetModuleBuildtree(self.SetModuleModel, self.rebuildtree)
            self.SetModuleTreeView.setModel(self.SetModuleModel)
            self.SetModuleTreeView.setHeaderHidden(True)
            self.RelieveModuleModel.itemChanged.connect(lambda: self.checkboxclick(self.RelieveModuleModel, RelieveModuleTreeInform))
            self.SetModuleModel.itemChanged.connect(lambda: self.checkboxclick(self.SetModuleModel, SetModuleTreeInform))
            self.CopyProcessValid = True
            self.plainTextEdit_Install.setPlainText("Installed MCAL model is loaded successfully.")
        else:            
            self.SetModuleModel.clear()
            self.RelieveModuleModel.clear()
            self.CopyProcessValid = False
            self.plainTextEdit_Install.setPlainText("Loading failed for Installed MCAL model.")
